...
function TrackInspector({ trackId, open, onOpenChange }: { trackId: number | null; open: boolean; onOpenChange: (value: boolean) => void }) {
  ...
  const play = async () => {
    ...
    const [audioUrl, artworkUrl, waveform] = await Promise.all([
      ...
      track.artwork_url ? mediaUrl(track.artwork_url).catch(() => null) : Promise.resolve(null),
...
export function Vault({ query }: { query: string }) {
  const [selectedTrackId, setSelectedTrackId] = useState<number | null>(null); const openedFromQueue = useRef<number | null>(null)
